...
2. **Search for a rigid l-set.** Find generating directions whose walks form an *independent generating set* of the class group — together, when there are two or more directions of order > 2, with a "sum"/pinning element that fixes the relative orientation of the cycles (`disc_rigid_lset_search`). The pool has three kinds of directions:

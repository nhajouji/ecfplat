...
2. **Search for a rigid l-set.** Find primes `ℓ` whose `ℓ`-isogeny directions form an *independent generating set* of the class group — together, when there are two or more directions of order > 2, with a "sum"/pinning element that fixes the relative orientation of the cycles. `disc_rigid_lset_search` (ordinary) and `ss_rigid_lset` (supersingular) do this, preferring cheap directions. ⚠️ *Failure mode:* the available prime pool may not generate the class group, so no rigid l-set exists in it.
3. **Collect neighbour data.** Build `ℓ`-isogeny adjacency on both sides for each chosen `ℓ`:
   - *Lattice side* — the class-group action on quadratic forms (pure form arithmetic).
   - *Curve side* — for the 15 genus-0 supersingular primes `ℓ ∈ {2, …, 71}` this is read off Atkin modular polynomials, with no isogeny computed explicitly; for any other `ℓ`, and throughout the supersingular case, the isogeny is computed explicitly with **Vélu's formulas**. ⚠️ *Failure mode:* Vélu needs an `ℓ`-torsion kernel that lives over an extension `F_{p^k}`, and when `k` is large this is too expensive — such directions are rejected as too costly (the l-set search tries to avoid them, which is why a valid l-set can still be out of practical reach).
...
Restricting to isogenies and endomorphisms defined over `F_p`, the supersingular classes are again an *oriented CM* picture — by an order in `ℚ(√−p)` — so the same class-group machinery applies. The objects are **signatures** `(j, s)`, where `s = ±1` distinguishes a curve from its quadratic twist (the F_p-isomorphism class). Frobenius has trace 0, so all curve-side neighbour data comes from Vélu: split-eigenline `ℓ`-isogenies for odd `ℓ`, and rational 2-torsion 2-isogenies for the volcano structure. The driver `ecqf_full_bijection_ss(p)` dispatches on `p mod 8`: no volcano when `p ≡ 1 (mod 4)`, and a surface/floor 2-volcano (sizes `1:1` for `p ≡ 7 (mod 8)`, `1:3` for `p ≡ 3 (mod 8)`) otherwise. `ss_bij_cache.py` rebuilds the whole supersingular dataset from scratch.

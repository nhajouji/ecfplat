...
```mermaid
flowchart TD
    A(["(a, p)"]) --> B["d = a² − 4p<br/>discriminant of the CM order"]
    B --> C{"rigid l-set<br/>search"}
    C -- "pool can't generate<br/>the class group" --> X1["✗ no rigid l-set"]
    C -- "found" --> D["collect ℓ-isogeny<br/>neighbour data"]
    D --> L["lattice side:<br/>quadratic forms"]
    D --> R["curve side:<br/>Atkin polynomials or Vélu"]
    R -- "kernel extension<br/>field too large" --> X2["✗ direction too costly"]
    L --> M["match the two labellings<br/>coordinate-by-coordinate"]
    R --> M
